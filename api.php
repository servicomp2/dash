<?php
/**
 * DASH-API v1                  License: MIT
 * Creado por: Cesar Rojas servicomp.cesar@gmail.com
 * Fecha de creacion: 04/04/2026
 * Mejorado para PHP 8.4+ y con nuevas funcionalidades
 * 
 * Basado en PHP CRUD API de Maurits van der Schee: maurits@vdschee.nl
...
namespace Nyholm\Psr7\Factory {
     ...
     *
     * @final This class
    class should never be extended. See https://github.com/Nyholm/psr7/blob/master/doc/final.md
     */
    ...
    class Psr17Factory implements RequestFactoryInterface, ResponseFactoryInterface, ServerRequestFactoryInterface, StreamFactoryInterface, UploadedFileFactoryInterface, UriFactoryInterface
    {
        ...
        {
            if (2 > \func_num_args()) {
                // This will make the Response class
                 // class to use a custom reasonPhrase
                $reasonPhrase = null;
            }
    ...
    class GenericReflection
    {
        ...
        private function getTablesSQL(): string
        {
            switch ($this->driver) {
                ...
                case 'pgsql':
                    return 'SELECT c.relname as "TABLE_NAME", c.relkind as "TABLE_TYPE" FROM pg_catalog.pg_#[\AllowDynamicProperties]
    class c WHERE c.relkind IN (\'r\', \'v\') AND c.relnamespace::regnamespace::text !~ \'^pg_|information_schema\' AND \'\' <> ? ORDER BY "TABLE_NAME";';
                case 'sqlsrv':
    ...
    class pgc ON pgc.oid = a.attrelid WHERE pgc.relname = ? AND \'\' <> ? AND pgc.relnamespace::regnamespace::text !~ \'^pg_|information_schema\' AND a.attnum > 0 AND NOT a.attisdropped ORDER BY a.attnum;';
        ...
        private function getTablePrimaryKeysSQL(): string
        {
            switch ($this->driver) {
                ...
                case 'pgsql':
                    return 'SELECT a.attname AS "COLUMN_NAME" FROM pg_attribute a JOIN pg_constraint c ON (c.conrelid, c.conkey[1]) = (a.attrelid, a.attnum) JOIN pg_#[\AllowDynamicProperties]
    class pgc ON pgc.oid = a.attrelid WHERE pgc.relname = ? AND \'\' <> ? AND pgc.relnamespace::regnamespace::text !~ \'^pg_|information_schema\' AND c.contype = \'p\'';
                case 'sqlsrv':
        ...
        private function getTableForeignKeysSQL(): string
        {
            switch ($this->driver) {
                ...
                case 'pgsql':
                    return 'SELECT a.attname AS "COLUMN_NAME", c.confrelid::regclass::text AS "REFERENCED_TABLE_NAME" FROM pg_attribute a JOIN pg_constraint c ON (c.conrelid, c.conkey[1]) = (a.attrelid, a.attnum) JOIN pg_#[\AllowDynamicProperties]
    class pgc ON pgc.oid = a.attrelid WHERE pgc.relname = ? AND \'\' <> ? AND pgc.relnamespace::regnamespace::text !~ \'^pg_|information_schema\' AND c.contype  = \'f\'';
                case 'sqlsrv':
...
namespace Tqdev\PhpCrudApi\Database {
    ...
    class LazyPdo
    {
        ...
        #[\ReturnTypeWillChange]
        public function prepare(...$args): \PDOStatement|false
        {
...
namespace Tqdev\PhpCrudApi\Middleware {
    ...
    class DbAuthMiddleware extends Middleware
    {
        ...
        public function process(ServerRequestInterface $request, RequestHandlerInterface $next): ResponseInterface
        {
                ...
                if ($path == 'login') {
                    ...
                    foreach ($users as $user) {
                        if (password_verify($password, $user[$passwordColumnName]) == 1) {
                            ...
                            unset($user[$passwordColumnName]);
                            $_SESSION['user'] = $user;
                            $claims = array_merge(['sub' => $user[$this->getProperty('usernameColumn', 'username')]], $user);
                            $jwtSecret = $this->config->getProperty('jwtAuth.secret');
                            $jwtIssuer = $this->config->getProperty('jwtAuth.issuer', 'DashboardACG-API');
                            $jwtTtl = $this->config->getProperty('jwtAuth.ttl', '3600');
                            $token = JwtAuthMiddleware::generateToken($claims, $jwtSecret, $jwtIssuer, (int)$jwtTtl);
            ...
            $path = RequestUtils::getPathSegment($request, 1);
            $authenticationMode = $this->getProperty('mode', 'required');
            if (in_array($path, ['openapi', 'login'])) {
...
// file: src/Tqdev/PhpCrudApi/Mailer.php
namespace Tqdev\PhpCrudApi {
    ...
    class Mailer
    {
        ...
        {
            if (!getenv('SMTP_HOST')) return false;
            ...
            try {
                ...
                $mail->setFrom(getenv('SMTP_FROM'), 'DASH-API Alerts');
...
// file: src/Tqdev/PhpCrudApi/Middleware/AdvancedMiddlewares.php
namespace Tqdev\PhpCrudApi\Middleware {
    ...
    class LoggingMiddleware extends Middleware
    {
        ...
        private function ensureTable() {
            try {
                $this->db->query("CREATE TABLE IF NOT EXISTS api_logs (
        ...
        public function process(ServerRequestInterface $request, RequestHandlerInterface $next): ResponseInterface
        {
            ...
            $path = RequestUtils::getPathSegment($request, 1);
            $tableName = RequestUtils::getPathSegment($request, 2);
            $recordId = RequestUtils::getPathSegment($request, 3);
            
            // Capturar datos ANTES de que el stream se consuma (aunque getParsedBody suele estar cacheado)
            $inputData = $request->getParsedBody();
            
            $response = $next->handle($request);
            
            if ($path == 'records' && in_array($method, ['POST', 'PUT', 'PATCH', 'DELETE'])) {
                if ($response->getStatusCode() == 200 || $response->getStatusCode() == 201) {
                    $userEmail = 'guest';
                    if (isset($_SESSION['user']) && is_array($_SESSION['user'])) {
                        $userEmail = $_SESSION['user']['email'] ?? ($_SESSION['user']['username'] ?? 'user');
                    }
                    
                    if ($method == 'POST') {
                        $newId = $response->getBody()->getContents();
                        $response->getBody()->rewind();
                        if (is_numeric($newId) || (strlen($newId) > 0 && strlen($newId) < 100)) {
                            $recordId = $newId;
                        }
                    }

                    try {
                        $this->db->query("INSERT INTO api_logs (user_email, operation, table_name, record_id, data) VALUES (?, ?, ?, ?, ?)", [
                            $userEmail, 
                            $method, 
                            $tableName, 
                            $recordId, 
            ...
            // También guardamos en archivo para redundancia si está habilitado
            if (getenv('LOG_TRANSACTIONS') === 'true') {
                $logFile = getenv('LOG_FILE') ?: 'logs/api_transactions.log';
                $dir = dirname($logFile);
                if (!is_dir($dir)) @mkdir($dir, 0755, true);
                $ip = $_SERVER['REMOTE_ADDR'] ?? '127.0.0.1';
                $log = sprintf("[%s] %s %s - STATUS: %s - IP: %s\n", date('Y-m-d H:i:s'), $method, $request->getUri()->getPath(), $response->getStatusCode(), $ip);
    ...
    class ThrottlingMiddleware extends Middleware
    {
        ...
        private function ensureTable() {
            try {
                $this->db->query("CREATE TABLE IF NOT EXISTS login_attempts (ip VARCHAR(45) PRIMARY KEY, attempts INT, last_attempt TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)", []);
            } catch (\Exception $e) {}
        }
        public function process(ServerRequestInterface $request, RequestHandlerInterface $next): ResponseInterface
        {
            ...
            if ($path == 'login' && $request->getMethod() == 'POST') {
                $ip = $_SERVER['REMOTE_ADDR'] ?? '127.0.0.1';
                $stmt = $this->db->query("SELECT attempts, UNIX_TIMESTAMP(last_attempt) as last_ts FROM login_attempts WHERE ip = ?", [$ip]);
                $row = $stmt->fetch();
                $maxAttempts = (int)getenv('LOGIN_MAX_ATTEMPTS', 3);
                $lockoutTime = (int)getenv('LOGIN_LOCKOUT_TIME', 900);
                if ($row && $row['attempts'] >= $maxAttempts && (time() - $row['last_ts']) < $lockoutTime) {
                    return $this->responder->error(ErrorCode::TEMPORARY_OR_PERMANENTLY_BLOCKED, 'IP bloqueada temporalmente por seguridad (3 intentos fallidos).');
                }
                $response = $next->handle($request);
                if ($response->getStatusCode() != 200) {
                    $this->db->query("INSERT INTO login_attempts (ip, attempts, last_attempt) VALUES (?, 1, CURRENT_TIMESTAMP) ON DUPLICATE KEY UPDATE attempts = attempts + 1, last_attempt = CURRENT_TIMESTAMP", [$ip]);
                } else {
    ...
    class SoftDeleteMiddleware extends Middleware
    {
        private $reflection;
        public function __construct(Router $router, Responder $responder, Config $config, string $middleware, ReflectionService $reflection)
        {
                ...
                if ($this->reflection->hasTable($tableName)) {
                    ...
                    if ($table->hasColumn('deleted_at')) {
                        if ($method == 'GET') {
                            ...
                            $id = RequestUtils::getPathSegment($request, 3);
                            if ($id) {
                                Mailer::send("Alerta DASH-API: Borrado Lógico", "Se ha realizado un Soft Delete en la tabla '$tableName' para el registro con ID: $id.\nIP Origen: " . ($_SERVER['REMOTE_ADDR'] ?? 'desconocida'));
                                $request = $request->withMethod('PATCH')->withParsedBody(['deleted_at' => date('Y-m-d H:i:s')]);
                            }
                        }
                    }
                }
...
namespace Tqdev\PhpCrudApi\Middleware {
    ...
    {
        public static function generateToken(array $claims, string $secret, string $issuer, int $ttl): string
        {
        ...
        {
            $headerName = $this->getProperty('header', 'Authorization');
        ...
        public function process(ServerRequestInterface $request, RequestHandlerInterface $next): ResponseInterface
        {
            ...
            if (empty($_SESSION['claims']) && $authenticationMode == 'required') {
                return $this->responder->error(ErrorCode::AUTHENTICATION_REQUIRED, '');
            }
            return $next->handle($request);
        }
...
namespace Tqdev\PhpCrudApi\OpenApi {
    ...
    class OpenApiBuilder
    {
        ...
        public function build(ServerRequestInterface $request): OpenApiDefinition
        {
            ...
            $this->openapi->set("components|securitySchemes|bearerAuth|type", "http");
            $this->openapi->set("components|securitySchemes|bearerAuth|scheme", "bearer");
            $this->openapi->set("components|securitySchemes|bearerAuth|bearerFormat", "JWT");
            $this->openapi->set("security||bearerAuth", array());
            
            // Inyectar endpoint /login
            $this->openapi->set("paths|/login|post|tags|", "Auth");
            $this->openapi->set("paths|/login|post|summary", "Obtener Token JWT");
            $this->openapi->set("paths|/login|post|requestBody|content|application/json|schema|type", "object");
            $this->openapi->set("paths|/login|post|requestBody|content|application/json|schema|properties|email|type", "string");
            $this->openapi->set("paths|/login|post|requestBody|content|application/json|schema|properties|password|type", "string");
            $this->openapi->set("paths|/login|post|responses|200|description", "Login exitoso");
            $this->openapi->set("paths|/login|post|responses|200|content|application/json|schema|type", "object");
            $this->openapi->set("paths|/login|post|responses|200|content|application/json|schema|properties|access_token|type", "string");
            ...
            foreach ($config->getMiddlewares() as $middleware) {
                switch ($middleware) {
                    ...
                    case 'xml':
                        ...
                        break;
                    case 'json':
                        new Middleware\JsonMiddleware($router, $responder, $config, $middleware);
                        break;
                    case 'logging':
                        new Middleware\LoggingMiddleware($router, $responder, $config, $middleware, $db);
                        break;
                    case 'throttling':
                        new Middleware\ThrottlingMiddleware($router, $responder, $config, $middleware, $db);
                        break;
                    case 'softDelete':
                        new Middleware\SoftDeleteMiddleware($router, $responder, $config, $middleware, $reflection);
                        break;
                }
            }
...
namespace Tqdev\PhpCrudApi {
    ...
    // .env Loader - Carga de variables de entorno
